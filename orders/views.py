...
from .models import Order, Line, Price, CustomerName, BillAddress, ShipAddress, Part, Account, Purchase, SalesReport, \
    PurchaseReport, Category, Manager, Vendor, General, \
    PurchaseMonthly, SalesMonthly, SalesCustomer, PurchaseVendor, SalesManager, PurchaseDepartment, GeneralReport, \
    FoundryMaterial, FoundryLabor, FoundryOverhead, ExtrusionLabor, ExtrusionOverhead, FabricationLabor, \
    FabricationOverhead, AnodizingLabor, AnodizingOverhead, ShippingLabor, ShippingOverhead, \
    Billet, Log, \
...
from django.db.models import Avg, Sum, Count, Min, Max, Q, TextField
...
def index_chart(request):
    ...
    if not log and not billet:
        inventory_chart = [['7" billet', random.randint(1, 100)], ['9" billet', random.randint(1, 100)],
                           ['11" billet', random.randint(1, 100)], ['14" billet', random.randint(1, 100)],
                           ['cropped log', random.randint(1, 100)], ['homogenized log', random.randint(1, 100)],
                           ['unhomogenized log', random.randint(1, 100)]]
    else:
        li = []
        for i in billet:
    ...
    for i in shipping_data:
        ...
        dictionary_form[shipping_due] = [shipping_weight, float(shipping_due) - float(shipping_weight), shipping_due,
                                         i.get('id')]
    ...
    recent_receive = Order.objects.filter(required_date__range=[startdate, enddate]).values('required_date').annotate(
        total=Round(Sum('amount_received'))).order_by()
    recent_payment = Purchase.objects.filter(required_date__range=[startdate, enddate]).values(
        'required_date').annotate(total=Round(Sum('total'))).order_by()
    ...
    cht16 = Chart(
        ...
        {'title': {
            ...
            'credits': {
                'enabled': False}})
    return render(request, 'indexchart.html',
                  {'chart_list': [cht16], 'inventory_chart': inventory_chart, 'response': response, 'three': three,
                   'two': two, 'one': one})
...
def destroy_sales_report(request):
    ...
    return redirect('/order/indexsalesreport',
                    messages.success(request, 'Sales Report was successfully deleted.', 'alert-success'))


@login_required
def destroy_purchase_report(request):
    PurchaseReport.objects.all().delete()
    return redirect('/order/indexpurchasereport',
                    messages.success(request, 'Purchase Report was successfully deleted.', 'alert-success'))
...
def performance(request):
    ...
    for i in qs:
        ...
        Calc_Price = s3
        buttons = '<a href="/order/checksales/' + str(
            i.id) + '" class="btn ink-reaction btn-floating-action btn-info"><i class="md md-search"></i> </a>'
...
def foundry_chart(request):
    ...
    recent_summary_foundry_metal = recent_summary_initial.filter(Accountno=5100717) | recent_summary_initial.filter(
        Accountno=5100726) | \
                                   recent_summary_initial.filter(Accountno=5100716) | recent_summary_initial.filter(
        Accountno=5100710) | \
                                   recent_summary_initial.filter(Accountno=5100715) | recent_summary_initial.filter(
        Accountno=5100720) | \
                                   recent_summary_initial.filter(Accountno=5100719) | recent_summary_initial.filter(
        Accountno=5100714) | \
                                   recent_summary_initial.filter(Accountno=5100711) | recent_summary_initial.filter(
        Accountno=5100707) | \
                                   recent_summary_initial.filter(Accountno=5100712) | recent_summary_initial.filter(
        Accountno=5100718)
    recent_summary_metal = recent_summary_foundry_metal.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    ...
    recent_summary_foundry_labor = recent_summary_initial.filter(Accountno=5100100) | recent_summary_initial.filter(
        Accountno=5100115) | \
                                   recent_summary_initial.filter(Accountno=5100103) | recent_summary_initial.filter(
        Accountno=5100108) | \
                                   recent_summary_initial.filter(Accountno=5100125) | recent_summary_initial.filter(
        Accountno=5100104) | \
                                   recent_summary_initial.filter(Accountno=5100111) | recent_summary_initial.filter(
        Accountno=5100102) | \
                                   recent_summary_initial.filter(Accountno=5100110)
    recent_summary_labor = recent_summary_foundry_labor.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    ...
    recent_summary_foundry_overhead = recent_summary_initial.filter(Accountno=5100105) | recent_summary_initial.filter(
        Accountno=5100120) | \
                                      recent_summary_initial.filter(Accountno=5100109) | recent_summary_initial.filter(
        Accountno=5100126) | \
                                      recent_summary_initial.filter(Accountno=5100410) | recent_summary_initial.filter(
        Accountno=5100820) | \
                                      recent_summary_initial.filter(Accountno=5100889) | recent_summary_initial.filter(
        Accountno=5100902) | \
                                      recent_summary_initial.filter(Accountno=5100350) | recent_summary_initial.filter(
        Accountno=5100310) | \
                                      recent_summary_initial.filter(Accountno=5100359) | recent_summary_initial.filter(
        Accountno=5100363) | \
                                      recent_summary_initial.filter(Accountno=5100313) | recent_summary_initial.filter(
        Accountno=5100352) | \
                                      recent_summary_initial.filter(Accountno=5100364) | recent_summary_initial.filter(
        Accountno=5100360) | \
                                      recent_summary_initial.filter(Accountno=5100319) | recent_summary_initial.filter(
        Accountno=5100355) | \
                                      recent_summary_initial.filter(Accountno=5100312) | recent_summary_initial.filter(
        Accountno=5100320) | \
                                      recent_summary_initial.filter(Accountno=5100315) | recent_summary_initial.filter(
        Accountno=5100311) | \
                                      recent_summary_initial.filter(Accountno=5100322) | recent_summary_initial.filter(
        Accountno=5100316) | \
                                      recent_summary_initial.filter(Accountno=5100620) | recent_summary_initial.filter(
        Accountno=5100210) | \
                                      recent_summary_initial.filter(Accountno=5100220) | recent_summary_initial.filter(
        Accountno=5100230) | \
                                      recent_summary_initial.filter(Accountno=5100314) | recent_summary_initial.filter(
        Accountno=5100317) | \
                                      recent_summary_initial.filter(Accountno=5100321) | recent_summary_initial.filter(
        Accountno=5100351) | \
                                      recent_summary_initial.filter(Accountno=5100356) | recent_summary_initial.filter(
        Accountno=5100357) | \
                                      recent_summary_initial.filter(Accountno=5100362) | recent_summary_initial.filter(
        Accountno=5100510) | \
                                      recent_summary_initial.filter(Accountno=5100610) | recent_summary_initial.filter(
        Accountno=5100615) | \
                                      recent_summary_initial.filter(Accountno=5100845) | recent_summary_initial.filter(
        Accountno=5100862)
    recent_summary_overhead = recent_summary_foundry_overhead.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    ...
    def glname(gl_num):
        names = {
            '5100100': 'Direct Labor Salaries (FOUNDRY)', '5100115': 'Health And Welfare (FOUNDRY)',
            '5100103': 'Overtime/Double Time (FOUNDRY)', '5100108': '!W/C INS - Direct Labor (FOUNDRY)',
            '5100125': 'Payroll Taxes Direct Labor (FOUNDRY)', '5100104': 'Direct Labor Vacation (FOUNDRY)',
            '5100111': 'Union 401K (FOUNDRY)', '5100102': 'Direct Labor Incentives (FOUNDRY)',
            '5100110': 'Union Pension (FOUNDRY)', '5100717': 'Purchases Scrap (FOUNDRY)',
            '5100726': 'Purchase - Extrusion Scrap (FOUNDRY)', '5100716': 'Purchases Pig (FOUNDRY)',
            '5100710': 'Purchases Log & Billet (FOUNDRY)', '5100715': 'Purchases Magnesium (FOUNDRY)',
            '5100720': 'Purchases Titaniun & Tibor (FOUNDRY)', '5100719': 'Purchases Silicon (FOUNDRY)',
            '5100714': 'Purchases Miscellaneous (FOUNDRY)', '5100711': 'Purchases Copper (FOUNDRY)',
            '5100707': 'Metal Hedging (Gain) / Loss (FOUNDRY)', '5100712': 'Purchases Dross Conversion (FOUNDRY)',
            '5100718': 'Purchases Rsi (FOUNDRY)', '5100105': 'Indirect Salaries Supervision (FOUNDRY)',
            '5100120': 'Hospitalization (FOUNDRY)', '5100109': '!W/C INS - IND DIR LABOR (FOUNDRY)',
            '5100126': 'Payroll Taxes Indrect Labor (FOUNDRY)', '5100410': 'Equipment Rental (FOUNDRY)',
            '5100820': 'Insurance Expense (FOUNDRY)', '5100889': 'Placement Fee (FOUNDRY)',
            '5100902': 'Depreciation Exp - M & E (FOUNDRY)', '5100350': 'Maintenance & Repair (FOUNDRY)',
            '5100310': 'Shop Supplies (FOUNDRY)', '5100359': 'Argon (FOUNDRY)',
            '5100363': 'Refractory Repairs (FOUNDRY)',
            '5100313': 'Wagstaff Supplies & Equip (FOUNDRY)', '5100352': 'Lift Truck Repairs (FOUNDRY)',
            '5100364': 'Outside Mechanical (FOUNDRY)', '5100360': 'Oven Repairs (FOUNDRY)',
            '5100319': 'Thermocouple Tubes (FOUNDRY)', '5100355': 'Gasoline (FOUNDRY)',
            '5100312': 'Uniform Expense (FOUNDRY)', '5100320': 'Hardware And Mill (FOUNDRY)',
            '5100315': 'Filtration Supplies (FOUNDRY)', '5100311': 'Saw Blades Expense (FOUNDRY)',
            '5100322': 'Gloves (FOUNDRY)', '5100316': 'Lubricants (FOUNDRY)', '5100620': 'Entertainment (FOUNDRY)',
            '5100210': 'Office Supplies (FOUNDRY)', '5100220': 'Furniture & Fixtures Expense (FOUNDRY)',
            '5100230': 'Computer Expense (FOUNDRY)', '5100314': 'Degasser Flux (FOUNDRY)',
            '5100317': 'Mach & Fabtn-O/S (FOUNDRY)', '5100321': 'Electrical Expense (FOUNDRY)',
            '5100351': 'Crane Repairs (FOUNDRY)', '5100356': 'Pump Repairs (FOUNDRY)',
            '5100357': 'Motor Repair (FOUNDRY)',
            '5100362': 'Furnace Doors & Jambs (FOUNDRY)', '5100510': 'Heat, Light & Power (FOUNDRY)',
            '5100610': 'Travel Expense (FOUNDRY)', '5100615': 'Buying Expense (FOUNDRY)',
    ...
    summary_foundry_metal = summary_initial.filter(Accountno=5100717) | summary_initial.filter(Accountno=5100726) | \
                            ...
                            summary_initial.filter(Accountno=5100711) | summary_initial.filter(Accountno=5100707) | \
                            summary_initial.filter(Accountno=5100712) | summary_initial.filter(Accountno=5100718)
    summary_metal = summary_foundry_metal.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    metal = DataPool(
        series=
        [{'options': {
    ...
    summary_foundry_labor = summary_initial.filter(Accountno=5100100) | summary_initial.filter(Accountno=5100115) | \
                            ...
                            summary_initial.filter(Accountno=5100111) | summary_initial.filter(Accountno=5100102) | \
                            summary_initial.filter(Accountno=5100110)
    summary_labor = summary_foundry_labor.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    labor = DataPool(
        series=
        [{'options': {
    ...
    summary_foundry_overhead = summary_initial.filter(Accountno=5100105) | summary_initial.filter(Accountno=5100120) | \
                               ...
                               summary_initial.filter(Accountno=5100610) | summary_initial.filter(Accountno=5100615) | \
                               summary_initial.filter(Accountno=5100845) | summary_initial.filter(Accountno=5100862)
    summary_overhead = summary_foundry_overhead.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    overhead = DataPool(
        series=
        [{'options': {
    ...
    return render(request, 'foundrychart.html',
                  {'chart_list': [cht, cht2, cht3, cht4, cht5, cht6, cht7, cht8, cht9, cht10, cht11, cht12, cht13,
                                  cht14, cht15, cht16, cht17, cht18], 'response': response})
...
def extrusion_chart(request):
    ...
    recent_summary_extrusion_total = recent_summary_initial.filter(
        Accountno__gte=5400000) & recent_summary_initial.filter(Accountno__lt=5410000)
    recent_summary_total = recent_summary_extrusion_total.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    ...
    recent_summary_extrusion_labor = recent_summary_initial.filter(Accountno=5400100) | recent_summary_initial.filter(
        Accountno=5400115) | \
                                     recent_summary_initial.filter(Accountno=5400103) | recent_summary_initial.filter(
        Accountno=5400108) | \
                                     recent_summary_initial.filter(Accountno=5400125) | recent_summary_initial.filter(
        Accountno=5400104) | \
                                     recent_summary_initial.filter(Accountno=5400111) | recent_summary_initial.filter(
        Accountno=5400102) | \
                                     recent_summary_initial.filter(Accountno=5400110)
    recent_summary_labor = recent_summary_extrusion_labor.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    recent_labor = DataPool(
        series=
        [{'options': {
    ...
    recent_summary_extrusion_overhead = recent_summary_initial.filter(
        Accountno=5400105) | recent_summary_initial.filter(Accountno=5400350) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400109) | recent_summary_initial.filter(Accountno=5400126) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400120) | recent_summary_initial.filter(Accountno=5400334) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400310) | recent_summary_initial.filter(Accountno=5400311) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400352) | recent_summary_initial.filter(Accountno=5400333) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400322) | recent_summary_initial.filter(Accountno=5400332) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400762) | recent_summary_initial.filter(Accountno=5400364) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400351) | recent_summary_initial.filter(Accountno=5400357) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400365) | recent_summary_initial.filter(Accountno=5400331) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400321) | recent_summary_initial.filter(Accountno=5400354) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400320) | recent_summary_initial.filter(Accountno=5400410) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400211) | recent_summary_initial.filter(Accountno=5400220) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400356) | recent_summary_initial.filter(Accountno=5400358) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400359) | recent_summary_initial.filter(Accountno=5400362) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400889) | recent_summary_initial.filter(Accountno=5400000) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400230) | recent_summary_initial.filter(Accountno=5400353) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400361) | recent_summary_initial.filter(Accountno=5400806) | \
                                        recent_summary_initial.filter(
                                            Accountno=5400210) | recent_summary_initial.filter(Accountno=5400335)
    recent_summary_overhead = recent_summary_extrusion_overhead.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    recent_overhead = DataPool(
        series=
        [{'options': {
    ...
    def glname(gl_num):
        names = {
            '5400100': 'Direct Labor Salaries (EXTRUSION)', '5400115': 'Health And Welfare (EXTRUSION)',
            '5400103': 'Overtime/Double Time (EXTRUSION)', '5400108': '!W/C INS - Direct Labor (EXTRUSION)',
            '5400125': 'Payroll Taxes Direct Labor (EXTRUSION)', '5400104': 'Direct Labor Vacation (EXTRUSION)',
            '5400111': 'Union 401K (EXTRUSION)', '5400102': 'Direct Labor Incentives (EXTRUSION)',
            '5400110': 'Union Pension (EXTRUSION)', '5400105': 'Indirect Salaries Supervision (EXTRUSION)',
            '5400350': 'Maintenance & Repairs (EXTRUSION)', '5400109': '!W/C INS - IND DIR LABOR (EXTRUSION)',
            '5400126': 'Payroll Taxes Indirect Labor (EXTRUSION)', '5400120': 'Hospitalization (EXTRUSION)',
            '5400334': 'Oil & Fluids (EXTRUSION)', '5400310': 'Shop Supplies (EXTRUSION)',
            '5400311': 'Saw Blades (EXTRUSION)', '5400352': 'Lift Truck Expense (EXTRUSION)',
            '5400333': 'Lubricants (EXTRUSION)', '5400322': 'Gloves & Personal Equip (EXTRUSION)',
            '5400332': 'Nitrogen (EXTRUSION)', '5400762': 'STRAPPING,PACKAGING (EXTRUSION)',
            '5400364': 'Outside Mechanical (EXTRUSION)', '5400351': 'Crane Expense (EXTRUSION)',
            '5400357': 'Motor Repair (EXTRUSION)', '5400365': 'Outside Electrical (EXTRUSION)',
            '5400331': 'Filters (EXTRUSION)', '5400321': 'Electrical Expense (EXTRUSION)',
            '5400354': 'Quality Control (EXTRUSION)', '5400320': 'Hardware And Mill (EXTRUSION)',
            '5400410': 'Equipment Rental (EXTRUSION)', '5400211': 'Commissions (Extrusions)',
            '5400220': 'Furniture & Fixtures Expense (EXTRUSION)', '5400356': 'Pump Repair (EXTRUSION)',
            '5400358': 'Billet Heater Repairs (EXTRUSION)', '5400359': 'Handling Equipment Repairs (EXTRUSION)',
            '5400362': 'Fixed Heads (EXTRUSION)', '5400889': 'Placement Fee (EXTRUSION)', '5400000': '5400000',
            '5400230': 'Computer Expense (EXTRUSION)', '5400353': 'Racking Expense (EXTRUSION)',
            '5400361': 'Aliminum Cutting (EXTRUSION)', '5400806': 'CONSULTING/TEMPORARY PERSONNEL (EXTRUSION)',
            '5400210': 'Office Supplies (EXTRUSION)', '5400335': 'Steel Expense (EXTRUSION)'
        }
        return names[gl_num]

    # start all time foundry datapool
    summary_initial = GeneralReport.objects.filter(Transamnt__gt=0)
    summary_extrusion_total = summary_initial.filter(Accountno__gte=5400000) & summary_initial.filter(
        Accountno__lt=5410000)
    summary_total = summary_extrusion_total.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    total = DataPool(
        series=
        [{'options': {
    ...
    summary_extrusion_labor = summary_initial.filter(Accountno=5400100) | summary_initial.filter(Accountno=5400115) | \
                              ...
                              summary_initial.filter(Accountno=5400111) | summary_initial.filter(Accountno=5400102) | \
                              summary_initial.filter(Accountno=5400110)
    summary_labor = summary_extrusion_labor.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    labor = DataPool(
        series=
        [{'options': {
    ...
    summary_extrusion_overhead = summary_initial.filter(Accountno=5400105) | summary_initial.filter(Accountno=5400350) | \
                                 ...
                                 summary_initial.filter(Accountno=5400361) | summary_initial.filter(Accountno=5400806) | \
                                 summary_initial.filter(Accountno=5400210) | summary_initial.filter(Accountno=5400335)
    summary_overhead = summary_extrusion_overhead.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    overhead = DataPool(
        series=
        [{'options': {
...
def filter_fabrication3(request):
    summary_initial = GeneralReport.objects.filter(Transamnt__gt=0)
    summary_fabrication_overhead = summary_initial.filter(Accountno=5650105) | summary_initial.filter(
        Accountno=5650120) | \
                                   summary_initial.filter(Accountno=5650109) | summary_initial.filter(
        Accountno=5650126) | \
                                   summary_initial.filter(Accountno=5650350) | summary_initial.filter(
        Accountno=5650310) | \
                                   summary_initial.filter(Accountno=5650309) | summary_initial.filter(
        Accountno=5650311) | \
                                   summary_initial.filter(Accountno=5650352) | summary_initial.filter(
        Accountno=5650410) | \
                                   summary_initial.filter(Accountno=5650106) | summary_initial.filter(
        Accountno=5650220) | \
                                   summary_initial.filter(Accountno=5650308) | summary_initial.filter(
        Accountno=5650357) | \
                                   summary_initial.filter(Accountno=5650368) | summary_initial.filter(
        Accountno=5650112) | \
                                   summary_initial.filter(Accountno=5650321) | summary_initial.filter(
        Accountno=5650210) | \
                                   summary_initial.filter(Accountno=5650320) | summary_initial.filter(
        Accountno=5650323) | \
...
def fabrication_chart(request):
    ...
    recent_summary_material = recent_summary_fabrication_material.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    ...
    recent_summary_fabrication_labor = recent_summary_initial.filter(Accountno=5650100) | recent_summary_initial.filter(
        Accountno=5650115) | \
                                       recent_summary_initial.filter(Accountno=5650103) | recent_summary_initial.filter(
        Accountno=5650108) | \
                                       recent_summary_initial.filter(Accountno=5650125) | recent_summary_initial.filter(
        Accountno=5650104) | \
                                       recent_summary_initial.filter(Accountno=5650111) | recent_summary_initial.filter(
        Accountno=5650102) | \
                                       recent_summary_initial.filter(Accountno=5650110)
    recent_summary_labor = recent_summary_fabrication_labor.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    recent_labor = DataPool(
        series=
        [{'options': {
    ...
    recent_summary_fabrication_overhead = recent_summary_initial.filter(
        Accountno=5650105) | recent_summary_initial.filter(Accountno=5650120) | \
                                          recent_summary_initial.filter(
                                              Accountno=5650109) | recent_summary_initial.filter(Accountno=5650126) | \
                                          recent_summary_initial.filter(
                                              Accountno=5650350) | recent_summary_initial.filter(Accountno=5650310) | \
                                          recent_summary_initial.filter(
                                              Accountno=5650309) | recent_summary_initial.filter(Accountno=5650311) | \
                                          recent_summary_initial.filter(
                                              Accountno=5650352) | recent_summary_initial.filter(Accountno=5650410) | \
                                          recent_summary_initial.filter(
                                              Accountno=5650106) | recent_summary_initial.filter(Accountno=5650220) | \
                                          recent_summary_initial.filter(
                                              Accountno=5650308) | recent_summary_initial.filter(Accountno=5650357) | \
                                          recent_summary_initial.filter(
                                              Accountno=5650368) | recent_summary_initial.filter(Accountno=5650112) | \
                                          recent_summary_initial.filter(
                                              Accountno=5650321) | recent_summary_initial.filter(Accountno=5650210) | \
                                          recent_summary_initial.filter(
                                              Accountno=5650320) | recent_summary_initial.filter(Accountno=5650323) | \
                                          recent_summary_initial.filter(Accountno=5650211)
    recent_summary_overhead = recent_summary_fabrication_overhead.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    recent_overhead = DataPool(
        series=
        [{'options': {
    ...
    def glname(gl_num):
        names = {
            '5650100': 'Direct Labor Salaries (FABRICATION)', '5650115': 'Health And Welfare (FABRICATION)',
            '5650103': 'Overtime/Double Time (FABRICATION)', '5650108': '!W/C INS - Direct Labor (FABRICATION)',
            '5650125': 'Payroll Taxes Direct Labor (FABRICATION)', '5650104': 'Direct Labor Vacation (FABRICATION)',
            '5650111': 'Union 401K (FABRICATION)', '5650102': 'Direct Labor Incentives (FABRICATION)',
            '5650110': 'Union Pension (FABRICATION)', '5650105': 'Indirect Salaries Supervision (FABRICATION)',
            '5650120': 'Hospitalization (FABRICATION)', '5650109': '!W/C INS - IND DIR LABOR (FABRICATION)',
            '5650126': 'Payroll Taxes Indirect Labor (FABRICATION)', '5650350': 'Maintenance & Repairs (FABRICATION)',
            '5650310': 'Shop Supplies (FABRICATION)', '5650309': 'Tooling Expense (FABRICATION)',
            '5650311': 'Saw Blades (FABRICATION)', '5650352': 'Lift Truck Repairs (FABRICATION)',
            '5650410': 'Equipment Rental (FABRICATION)', '5650106': 'Welding - Delair Services (FABRICATION)',
            '5650220': 'Furniture & Fixtures Expense (FABRICATION)', '5650308': 'Tooling Maintenance (FABRICATION)',
            '5650357': 'Motor Repair (FABRICATION)',
            '5650368': 'Equipment Repair (FABRICATION)', '5650112': 'Labor Transfer In/(Out) (FABRICATION)',
            '5650321': 'Electrical Expense (FABRICATION)',
            '5650210': 'Office Supplies (FABRICATION)', '5650320': 'Hardware And Mill (FABRICATION)',
            '5650323': 'Fence Assembly Misc Parts (FABRICATION)', '5650211': 'Salesman Commissions (FABRICATION)',
    ...
    summary_material = summary_fabrication_material.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    material = DataPool(
        series=
        [{'options': {
    ...
    summary_fabrication_labor = summary_initial.filter(Accountno=5650100) | summary_initial.filter(Accountno=5650115) | \
                                ...
                                summary_initial.filter(Accountno=5650111) | summary_initial.filter(Accountno=5650102) | \
                                summary_initial.filter(Accountno=5650110)
    summary_labor = summary_fabrication_labor.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    labor = DataPool(
        series=
        [{'options': {
    ...
    summary_fabrication_overhead = summary_initial.filter(Accountno=5650105) | summary_initial.filter(
        ...
        Accountno=5650323) | \
                                   summary_initial.filter(Accountno=5650211)
    summary_overhead = summary_fabrication_overhead.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    overhead = DataPool(
        series=
        [{'options': {
...
def anodizing_chart(request):
    ...
    recent_summary_material = recent_summary_anodizing_material.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    recent_material = DataPool(
        series=
        [{'options': {
    ...
    recent_summary_anodizing_labor = recent_summary_initial.filter(Accountno=5750100) | recent_summary_initial.filter(
        Accountno=5750115) | \
                                     recent_summary_initial.filter(Accountno=5750103) | recent_summary_initial.filter(
        Accountno=5750108) | \
                                     recent_summary_initial.filter(Accountno=5750125) | recent_summary_initial.filter(
        Accountno=5750104) | \
                                     recent_summary_initial.filter(Accountno=5750111) | recent_summary_initial.filter(
        Accountno=5750102) | \
                                     recent_summary_initial.filter(Accountno=5750110)
    recent_summary_labor = recent_summary_anodizing_labor.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    recent_labor = DataPool(
        series=
        [{'options': {
    ...
    recent_summary_anodizing_overhead = recent_summary_initial.filter(
        Accountno=5750105) | recent_summary_initial.filter(Accountno=5750350) | \
                                        recent_summary_initial.filter(
                                            Accountno=5750410) | recent_summary_initial.filter(Accountno=5750120) | \
                                        recent_summary_initial.filter(
                                            Accountno=5750310) | recent_summary_initial.filter(Accountno=5750109) | \
                                        recent_summary_initial.filter(
                                            Accountno=5750126) | recent_summary_initial.filter(Accountno=5750352) | \
                                        recent_summary_initial.filter(
                                            Accountno=5750211) | recent_summary_initial.filter(Accountno=5750900) | \
                                        recent_summary_initial.filter(
                                            Accountno=5750351) | recent_summary_initial.filter(Accountno=5750356) | \
                                        recent_summary_initial.filter(
                                            Accountno=5750357) | recent_summary_initial.filter(Accountno=5750364) | \
                                        recent_summary_initial.filter(
                                            Accountno=5750112) | recent_summary_initial.filter(Accountno=5750230) | \
                                        recent_summary_initial.filter(
                                            Accountno=5750321) | recent_summary_initial.filter(Accountno=5750610) | \
                                        recent_summary_initial.filter(
                                            Accountno=5750705) | recent_summary_initial.filter(Accountno=5750800) | \
                                        recent_summary_initial.filter(
                                            Accountno=5750806) | recent_summary_initial.filter(Accountno=5750210) | \
                                        recent_summary_initial.filter(
                                            Accountno=5750210) | recent_summary_initial.filter(Accountno=5750702) | \
                                        recent_summary_initial.filter(Accountno=5750510)
    recent_summary_overhead = recent_summary_anodizing_overhead.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    recent_overhead = DataPool(
        series=
        [{'options': {
    ...
    def glname(gl_num):
        names = {
            '5750100': 'Direct Labor Salaries (ANODIZING)', '5750115': 'Health And Welfare (ANODIZING)',
            '5750103': 'Overtime/Double Time (ANODIZING)', '5750108': '!W/C INS - Direct Labor (ANODIZING)',
            '5750125': 'Payroll Taxes Direct Labor (ANODIZING)', '5750104': 'Direct Labor Vacation (ANODIZING)',
            '5750111': 'Union 401K (ANODIZING)', '5750102': 'Direct Labor Incentives (ANODIZING)',
            '5750110': 'Union Pension (ANODIZING)', '5750105': 'Indirect Salaries Supervision (ANODIZING)',
            '5750350': 'Maintenance & Repair (ANODIZING)', '5750410': 'Equipment Rental (ANODIZING)',
            '5750120': 'Hospitalization (ANODIZING)', '5750310': 'Shop Supplies (ANODIZING)',
            '5750109': '!W/C INS - IND DIR LABOR (ANODIZING)', '5750126': 'Payroll Taxes Indirect Labor (ANODIZING)',
            '5750352': 'Racking Expense (ANODIZING)', '5750211': 'Salesman Commissions (ANODIZING)',
            '5750900': 'Depreciation Expense (ANODIZING)', '5750351': 'Equipment Repair (ANODIZING)',
            '5750356': 'Pump Repair (ANODIZING)', '5750357': 'Motor Repair (ANODIZING)',
            '5750364': 'Outside Mechanical (ANODIZING)', '5750112': 'Labor Transfer In/(Out) (ANODIZING)',
            '5750230': 'Computer Expense (ANODIZING)', '5750321': 'Electrical Expense (ANODIZING)',
            '5750610': 'Travel Expenses (ANODIZING)', '5750705': 'Outside Anodizing (ANODIZING)',
            '5750800': 'Hazardous Waste (ANODIZING)', '5750806': 'Temporary Employees (ANODIZING)',
            '5750210': 'Office Supplies (ANODIZING)', '5750702': 'Non-Production Chemicals (ANODIZING)',
    ...
    summary_material = summary_anodizing_material.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    material = DataPool(
        series=
        [{'options': {
    ...
    summary_anodizing_labor = summary_initial.filter(Accountno=5750100) | summary_initial.filter(Accountno=5750115) | \
                              ...
                              summary_initial.filter(Accountno=5750111) | summary_initial.filter(Accountno=5750102) | \
                              summary_initial.filter(Accountno=5750110)
    summary_labor = summary_anodizing_labor.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    labor = DataPool(
        series=
        [{'options': {
    ...
    summary_anodizing_overhead = summary_initial.filter(Accountno=5750105) | summary_initial.filter(Accountno=5750350) | \
                                 ...
                                 summary_initial.filter(Accountno=5750210) | summary_initial.filter(Accountno=5750702) | \
                                 summary_initial.filter(Accountno=5750510)
    summary_overhead = summary_anodizing_overhead.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    overhead = DataPool(
        series=
        [{'options': {
...
def shipping_chart(request):
    ...
    recent_summary_shipping_total = recent_summary_initial.filter(
        Accountno__gte=5800000) & recent_summary_initial.filter(Accountno__lt=5801000)
    recent_summary_total = recent_summary_shipping_total.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    recent_total = DataPool(
        series=
        [{'options': {
    ...
    recent_summary_shipping_labor = recent_summary_initial.filter(Accountno=5800100) | recent_summary_initial.filter(
        Accountno=5800115) | \
                                    recent_summary_initial.filter(Accountno=5800103) | recent_summary_initial.filter(
        Accountno=5800108) | \
                                    recent_summary_initial.filter(Accountno=5800125) | recent_summary_initial.filter(
        Accountno=5800104) | \
                                    recent_summary_initial.filter(Accountno=5800111) | recent_summary_initial.filter(
        Accountno=5800102) | \
                                    recent_summary_initial.filter(Accountno=5800110)
    recent_summary_labor = recent_summary_shipping_labor.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    recent_labor = DataPool(
        series=
        [{'options': {
    ...
    recent_summary_shipping_overhead = recent_summary_initial.filter(Accountno=5800105) | recent_summary_initial.filter(
        Accountno=5800761) | \
                                       recent_summary_initial.filter(Accountno=5800845) | recent_summary_initial.filter(
        Accountno=5800120) | \
                                       recent_summary_initial.filter(Accountno=5800760) | recent_summary_initial.filter(
        Accountno=5800109) | \
                                       recent_summary_initial.filter(Accountno=5800762) | recent_summary_initial.filter(
        Accountno=5800310) | \
                                       recent_summary_initial.filter(Accountno=5800126) | recent_summary_initial.filter(
        Accountno=5800352) | \
                                       recent_summary_initial.filter(Accountno=5800350) | recent_summary_initial.filter(
        Accountno=5800410) | \
                                       recent_summary_initial.filter(Accountno=5800889) | recent_summary_initial.filter(
        Accountno=5800112) | \
                                       recent_summary_initial.filter(Accountno=5800210) | recent_summary_initial.filter(
        Accountno=5800311) | \
                                       recent_summary_initial.filter(Accountno=5800321) | recent_summary_initial.filter(
        Accountno=5800364) | \
                                       recent_summary_initial.filter(Accountno=5800351) | recent_summary_initial.filter(
        Accountno=5800357) | \
                                       recent_summary_initial.filter(Accountno=5800806) | recent_summary_initial.filter(
        Accountno=5800220) | \
                                       recent_summary_initial.filter(Accountno=5800320)
    recent_summary_overhead = recent_summary_shipping_overhead.values('Accountno').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    recent_overhead = DataPool(
        series=
        [{'options': {
    ...
    def glname(gl_num):
        names = {
            '5800100': 'Direct Labor Salaries (SHIPPING)', '5800115': 'Health And Welfare (SHIPPING)',
            '5800103': 'Overtime/Double Time (SHIPPING)', '5800108': '!W/C INS - Direct Labor (SHIPPING)',
            '5800125': 'Payroll Taxes Direct Labor (SHIPPING)', '5800104': 'Direct Labor Vacation (SHIPPING)',
            '5800111': 'Union 401K (SHIPPING)', '5800102': 'Direct Labor Incentives (SHIPPING)',
            '5800110': 'Union Pension (SHIPPING)', '5800105': 'Indirect Salaries Supervision (SHIPPING)',
            '5800761': 'Lumber & Packing (SHIPPING)', '5800845': 'Truck And Auto (SHIPPING)',
            '5800120': 'Hospitalization (SHIPPING)', '5800760': 'Corrugated & Paper (SHIPPING)',
            '5800109': '!W/C INS - IND DIR LABOR (SHIPPING)', '5800762': 'Steel Strapping & Tape (SHIPPING)',
            '5800310': 'Shop Supplies (SHIPPING)', '5800126': 'Payroll Taxes Indirect Labor (SHIPPING)',
            '5800352': 'Lift Truck Repairs (SHIPPING)', '5800350': 'Maintenance & Repairs (SHIPPING)',
            '5800410': 'Equipment Rental (SHIPPING)', '5800889': 'Placement Fee (SHIPPING)',
            '5800112': 'Labor Transfer In/(Out) (SHIPPING)', '5800210': 'Office Supplies (SHIPPING)',
            '5800311': 'Saw Blades (SHIPPING)', '5800321': 'Electrical Expense (SHIPPING)', '5800364': '5800364',
            '5800351': 'Crane Repair (SHIPPING)', '5800357': 'Motor Repair (SHIPPING)',
            '5800806': 'Temporary Employees (SHIPPING)', '5800220': 'Furniture & Fixtures Expense (SHIPPING)',
            '5800320': 'Hardware And Mill (SHIPPING)'
        }
        return names[gl_num]

    # start all time foundry datapool
    summary_initial = GeneralReport.objects.filter(Transamnt__gt=0)
    summary_shipping_total = summary_initial.filter(Accountno__gte=5800000) & summary_initial.filter(
        Accountno__lt=5801000)
    summary_total = summary_shipping_total.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    total = DataPool(
        series=
        [{'options': {
    ...
    summary_shipping_labor = summary_initial.filter(Accountno=5800100) | summary_initial.filter(Accountno=5800115) | \
                             ...
                             summary_initial.filter(Accountno=5800111) | summary_initial.filter(Accountno=5800102) | \
                             summary_initial.filter(Accountno=5800110)
    summary_labor = summary_shipping_labor.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    labor = DataPool(
        series=
        [{'options': {
    ...
    summary_shipping_overhead = summary_initial.filter(Accountno=5800105) | summary_initial.filter(Accountno=5800761) | \
                                ...
                                summary_initial.filter(Accountno=5800806) | summary_initial.filter(Accountno=5800220) | \
                                summary_initial.filter(Accountno=5800320)
    summary_overhead = summary_shipping_overhead.annotate(month=TruncMonth('Transdate')).values('month').annotate(
        total=Round(Sum('Transamnt'))).order_by()
    overhead = DataPool(
        series=
        [{'options': {
...
def paint_chart(request):
    ...
    enddate = datetime.today()
    startdate = enddate - timedelta(days=90)
    recent_summary_initial = PurchaseReport.objects.filter(Datepromised__range=[startdate, enddate]).filter(
        Line_Amount__gt=0)

    recent_summary_paint_total = recent_summary_initial.filter(Vendor_Name__contains='POWDER')
    recent_summary_total = recent_summary_paint_total.values('GL_Accountno').annotate(
        total=Round(Sum('Line_Amount'))).order_by()
    recent_total = DataPool(
        series=
        [{'options': {
            'source': recent_summary_total},
    ...
    summary_total = summary_paint_total.annotate(month=TruncMonth('Datepromised')).values('month').annotate(
        total=Round(Sum('Line_Amount'))).order_by()
    total = DataPool(
        series=
        [{'options': {
...
def sales_chart(request):
    ...
    def monthname(month_num):
        ...
        return names[int(month_num)]

    # start past three month sales datapool
    enddate = datetime.today()
    startdate = enddate - timedelta(days=90)
    recent_summary = SalesReport.objects.filter(Invoicedate__range=[startdate, enddate]).annotate(
        month=TruncMonth('Invoicedate')).values('month').annotate(total=Round(Sum('Calc_Price'))).order_by()
    ...
    recent_company = SalesReport.objects.filter(Invoicedate__range=[startdate, enddate]).values('Companyname').annotate(
        total=Round(Sum('Calc_Price'))).order_by()
    recent_customer = DataPool(
        series=
        [{'options': {
            #    'source': SalesReport.objects.all()},
    ...
    recent_rsm = SalesReport.objects.filter(Invoicedate__range=[startdate, enddate]).values('RSM_name').annotate(
        total=Round(Sum('Calc_Price'))).order_by()
    ...
    summary = SalesReport.objects.annotate(month=TruncMonth('Invoicedate')).values('month').annotate(
        total=Round(Sum('Calc_Price'))).order_by()
    sales = DataPool(
        series=
        [{'options': {
    ...
    return render(request, 'saleschart.html',
                  {'chart_list': [cht, cht2, cht3, cht4, cht5, cht6, cht7, cht8, cht9, cht10, cht11, cht12, cht13,
                                  cht14, cht15, cht16, cht17, cht18, cht19, cht20, cht21], 'response': response})
...
def purchase_chart(request):
    ...
    def glname(gl_num):
        gl_names = {
            '1500110': 'BUILDING IMPROVEMENTS (CAPITAL EXPENSE AND MISC.)',
            '1500115': 'BUILDING IMPROVEMENTS - ANODIZING (CAPITAL EXPENSE AND MISC.)',
            '1500300': 'M & E  - DIES (CAPITAL EXPENSE AND MISC.)',
            '1500310': 'M & E  - DIE TOOLING (CAPITAL EXPENSE AND MISC.)',
            '1500315': 'FABRICATION DIES & TOOLING (CAPITAL EXPENSE AND MISC.)',
            '1500320': 'M & E  - SHAPES (CAPITAL EXPENSE AND MISC.)',
            '1500323': 'M & E  - FOUNDRY (CAPITAL EXPENSE AND MISC.)',
            '1500325': 'M & E DANIELI PRESS (CAPITAL EXPENSE AND MISC.)',
            ...
            '5100210': 'Office Supplies (FOUNDRY)', '5100310': 'Shop Supplies (FOUNDRY)',
            '5100311': 'Saw Blades Expense (FOUNDRY)', '5100312': 'Uniform Expense (FOUNDRY)',
            '5100313': 'Wagstaff Supplies & Equip (FOUNDRY)', '5100315': 'Filtration Supplies (FOUNDRY)',
            '5100316': 'Lubricants (FOUNDRY)', '5100317': 'Mach & Fabtn-O/S (FOUNDRY)',
            '5100318': '5100318', '5100319': 'Thermocouple Tubes (FOUNDRY)', '5100320': 'Hardware And Mill (FOUNDRY)',
            '5100321': 'Electrical Expense (FOUNDRY)',
            '5100322': 'Gloves (FOUNDRY)', '5100350': 'Maintenance & Repair (FOUNDRY)',
            '5100351': 'Crane Repairs (FOUNDRY)', '5100352': 'Lift Truck Repairs (FOUNDRY)',
            '5100355': 'Gasoline (FOUNDRY)', '5100359': 'Argon (FOUNDRY)', '5100360': 'Oven Repairs (FOUNDRY)',
            '5100363': 'Refractory Repairs (FOUNDRY)', '5100364': 'Outside Mechanical (FOUNDRY)',
            '5100410': 'Equipment Rental (FOUNDRY)', '5100710': 'Purchases Log & Billet (FOUNDRY)',
            '5100711': 'Purchases Copper (FOUNDRY)',
            '5100714': 'Purchases Miscellaneous (FOUNDRY)', '5100715': 'Purchases Magnesium (FOUNDRY)',
            '5100716': 'Purchases Pig (FOUNDRY)', '5100717': 'Purchases Scrap (FOUNDRY)',
            '5100719': 'Purchases Silicon (FOUNDRY)', '5100720': 'Purchases Titaniun & Tibor (FOUNDRY)',
            '5100725': 'Purchases Titaniun & Tibor (FOUNDRY)', '5100761': 'Freight Out - Finished Product (FOUNDRY)',
            '5100762': 'Freight Out - Finished Product (FOUNDRY)', '5300310': 'Shop Supplies (DIE CORRECTION)',
            '5300350': 'Maintenance & Repair (DIE CORRECTION)', '5300352': 'Lift Truck Expense (DIE CORRECTION)',
            '5300701': 'Chemicals (DIE CORRECTION)',
            '540000': '540000', '5400310': 'Shop Supplies (EXTRUSION)', '5400311': 'Saw Blades (EXTRUSION)',
            '5400320': 'Hardware And Mill (EXTRUSION)',
            '5400321': 'Electrical Expense (EXTRUSION)', '5400322': 'Gloves & Personal Equip (EXTRUSION)',
            '5400331': 'Filters (EXTRUSION)', '5400332': 'Nitrogen (EXTRUSION)', '5400333': 'Lubricants (EXTRUSION)',
            '5400334': 'Oil & Fluids (EXTRUSION)', '5400350': 'Maintenance & Repairs (EXTRUSION)',
            '5400351': 'Crane Expense (EXTRUSION)', '5400352': 'Lift Truck Expense (EXTRUSION)',
            '5400354': 'Quality Control (EXTRUSION)',
            '5400356': 'Pump Repair (EXTRUSION)', '5400357': 'Motor Repair (EXTRUSION)',
            '5400358': 'Billet Heater Repairs (EXTRUSION)', '5400359': 'Handling Equipment Repairs (EXTRUSION)',
            '5400364': 'Outside Mechanical (EXTRUSION)', '5400365': 'Outside Electrical (EXTRUSION)',
            '5400410': 'Equipment Rental (EXTRUSION)', '5400762': 'STRAPPING,PACKAGING,EXTRUSION (EXTRUSION)',
            '5650309': 'Tooling Expense (FABRICATION)', '5650310': 'Shop Supplies (FABRICATION)',
            '5650311': 'Saw Blades (FABRICATION)', '5650320': 'Hardware And Mill (FABRICATION)',
            '5650350': 'Maintenance & Repairs (FABRICATION)',
            '5650352': 'Lift Truck Repairs (FABRICATION)', '5650702': 'Purchase Fabrication', '569205': '569205',
            '569405': '569405', '569501': '569501', '5700350': 'Maintenance & Repair (PAINTLINE)', '5700701': '5700701',
            '570073': '570073',
            '5750310': 'Shop Supplies (ANODIZING)', '5750350': 'Maintenance & Repair (ANODIZING)',
            '5750352': 'Racking Expense (ANODIZING)', '5750364': 'Outside Mechanical (ANODIZING)',
            '5750410': 'Equipment Rental (ANODIZING)',
            '5750701': 'Purchase Chemicals (ANODIZING)', '5750705': 'Outside Anodizing (ANODIZING)',
            '5800210': 'Office Supplies (SHIPPING)',
            '5800310': 'Shop Supplies (SHIPPING)', '5800321': 'Electrical Expense (SHIPPING)',
            '5800350': 'Maintenance & Repairs (SHIPPING)',
            '5800352': 'Lift Truck Repairs (SHIPPING)', '5800760': 'Corrugated & Paper (SHIPPING)',
            '5800761': 'Lumber & Packing (SHIPPING)',
            '5800762': 'Steel Strapping & Tape (SHIPPING)', '5800845': 'Truck And Auto (SHIPPING)',
            '5900175': 'Christmas Expense (GENERAL PLANT)',
            '5900210': 'Office Supplies (GENERAL PLANT)', '5900310': 'Shop Supplies (GENERAL PLANT)',
            '5900312': 'Uniform Expense (GENERAL PLANT)',
            '5900320': 'Hardware And Mill (GENERAL PLANT)', '5900321': 'Electrical Expense (GENERAL PLANT)',
            '5900350': 'Maintenance & Repairs (GENERAL PLANT)',
            '5900351': 'Crane Repair (GENERAL PLANT)', '5900352': 'Lift Truck Repairs (GENERAL PLANT)',
            '5900355': 'Gas And Oil (GENERAL PLANT)',
            '5900360': 'Oven Repairs (GENERAL PLANT)', '5900361': 'Safety (GENERAL PLANT)',
            '5900364': 'Outside Mechanical (GENERAL PLANT)', '5900410': 'Equipment Rental (GENERAL PLANT)',
            '5900800': 'Hazardous Waste (GENERAL PLANT)', '5900810': '!ENVIROMENTAL EXP (GENERAL PLANT)',
            '5900862': 'TRAINING (GENERAL PLANT)', '5950210': 'Office Supplies (QUALITY CONTROL)',
            '5950310': 'Shop Supplies (QUALITY CONTROL)', '5950320': 'Hardware And Mill (QUALITY CONTROL)',
            '5950321': 'CALIBRATION (QUALITY CONTROL)', '5950350': 'Maintenance & Repairs (QUALITY CONTROL)',
            '5950610': 'Travel Expenses (QUALITY CONTROL)',
            '6010175': 'Christmas Expense (OFFICE)', '6010210': 'Office Supplies (OFFICE)',
            '6010230': 'Computer Expense Shapes (OFFICE)', '6010330': 'Cleaning Expense (OFFICE)',
            '6010350': 'Maintenance & Repairs (OFFICE)', '6010611': 'Transportation-Airline-Office (OFFICE)',
            '6010805': 'Legal, Audit & Professional (OFFICE)', '6010806': 'Consulting/Temporary Personnel (OFFICE)',
            '6010810': 'Environmental Expense (OFFICE)', '6010875': 'Stationary & Printing Expense (OFFICE)',
            '6010888': 'Telephone/Fax/Cell-Office (OFFICE)', '6020210': 'Office Supplies (SALES) ',
            '6020851': 'Advertising (SALES)', '6070230': 'COMPUTER EXP (IT)', 'E': 'E'}
        gl_names = {str(k): v for k, v in gl_names.items()}
        return gl_names[gl_num]

    # start past three month sales datapool
    enddate = datetime.today()
    startdate = enddate - timedelta(days=90)
    recent_summary = PurchaseReport.objects.filter(Datepromised__range=[startdate, enddate]).annotate(
        month=TruncMonth('Datepromised')).values('month').annotate(total=Round(Sum('Line_Amount'))).order_by()
    ...
    recent_company = PurchaseReport.objects.filter(Datepromised__range=[startdate, enddate]).values(
        'Vendor_Name').annotate(total=Round(Sum('Line_Amount'))).order_by()
    recent_customer = DataPool(
        series=
        [{'options': {
            'source': recent_company},
    ...
    recent_dep = PurchaseReport.objects.filter(Datepromised__range=[startdate, enddate]).values('GL_Accountno').annotate(GL_Accountno_str=Cast('GL_Accountno', TextField()), total=Round(Sum('Line_Amount'))).order_by()
    recent_department = DataPool(
        series=
        [{'options': {
            'source': recent_dep},
            'terms': [{'department': 'GL_Accountno_str',
                       'purchase': 'total'}]
        },
    ...
    summary = PurchaseReport.objects.annotate(month=TruncMonth('Datepromised')).values('month').annotate(
        total=Round(Sum('Line_Amount'))).order_by()
    purchase = DataPool(
        series=
        [{'options': {
    ...
    dep = PurchaseReport.objects.values('GL_Accountno').annotate(GL_Accountno_str=Cast('GL_Accountno', TextField()), total=Round(Sum('Line_Amount'))).order_by()
    department = DataPool(
        series=
        [{'options': {
            'source': dep},
        ...
        if form.is_valid():
            if form.save():
                return redirect('/orders',
                                messages.success(request, 'Order was successfully created.', 'alert-success'))
            else:
...
def newLine(request):
    if request.POST:
        ...
        if line.is_valid():
            if line.save():
                return redirect('/order/indexline',
                                messages.success(request, 'New Line was successfully created.', 'alert-success'))
            else:
...
def edit(request, order_id):
    ...
    if request.POST:
        ...
        if form.is_valid():
            if form.save():
                return redirect('/orders',
                                messages.success(request, 'Order was successfully updated.', 'alert-success'))
            else:
                return redirect('/orders', messages.error(request, 'Data is not saved', 'alert-danger'))
        else:
            print(form.errors)
...
def editLine(request, line_id):
    ...
    if request.POST:
        ...
        if line.is_valid():
            if line.save():
                return redirect('/order/indexline',
                                messages.success(request, 'New Line was successfully updated.', 'alert-success'))
            else:
                return redirect('/order/indexline', messages.error(request, 'Data is not saved', 'alert-danger'))
        else:
            return redirect('/order/indexline', messages.error(request, 'Line is not valid', 'alert-danger'))
...
def editPrice(request, price_id):
    ...
    if request.POST:
        ...
        if price.is_valid():
            if price.save():
                return redirect('/order/indexprice',
                                messages.success(request, 'New Price was successfully updated.', 'alert-success'))
            else:
    ...
    if request.POST:
        ...
        if customername.is_valid():
            if customername.save():
                return redirect('/order/indexcustomername',
                                messages.success(request, 'Customer Name was successfully created.', 'alert-success'))
            else:
                return redirect('/order/indexcustomername',
                                messages.error(request, 'Customer Name is not saved', 'alert-danger'))
        else:
            return redirect('/order/indexcustomername',
                            messages.error(request, 'Customer Name is not valid', 'alert-danger'))
    else:
...
def destroy_customer_name(request, customer_id):
    if CustomerName.objects.filter(id=customer_id).update(active='0'):
        return redirect('/order/indexcustomername',
                        messages.success(request, 'Customer Name was successfully deleted.', 'alert-success'))
    else:
        return redirect('/order/indexcustomername',
                        messages.danger(request, 'Cannot delete Customer Name while its order exists.', 'alert-danger'))
...
def new_bill_address(request):
    if request.POST:
        ...
        if billaddress.is_valid():
            if billaddress.save():
                return redirect('/order/indexbilladdress',
                                messages.success(request, 'Bill address was successfully created.', 'alert-success'))
            else:
                return redirect('/order/indexbilladdress',
                                messages.error(request, 'Bill address is not saved', 'alert-danger'))
        else:
            return redirect('/order/indexbilladdress',
                            messages.error(request, 'Bill address is not valid', 'alert-danger'))
    else:
...
def new_ship_address(request):
    if request.POST:
        ...
        if shipaddress.is_valid():
            if shipaddress.save():
                return redirect('/order/indexshipaddress',
                                messages.success(request, 'Ship address was successfully created.', 'alert-success'))
            else:
                return redirect('/order/indexshipaddress',
                                messages.error(request, 'Ship address is not saved', 'alert-danger'))
        else:
            return redirect('/order/indexshipaddress',
                            messages.error(request, 'Ship address is not valid', 'alert-danger'))
    else:
...
def destroy_bill_address(request, billAddress_id):
    if BillAddress.objects.filter(id=billAddress_id).update(active='0'):
        return redirect('/order/indexbilladdress',
                        messages.success(request, 'Bill Address was successfully deleted.', 'alert-success'))
    else:
        return redirect('/order/indexbilladdress',
                        messages.danger(request, 'Cannot delete Bill Address while its order exists.', 'alert-danger'))
...
def destroy_ship_address(request, shipAddress_id):
    if ShipAddress.objects.filter(id=shipAddress_id).update(active='0'):
        return redirect('/order/indexshipaddress',
                        messages.success(request, 'Ship Address was successfully deleted.', 'alert-success'))
    else:
        return redirect('/order/indexshipaddress',
                        messages.danger(request, 'Cannot delete Ship Address while its order exists.', 'alert-danger'))
...
def newPurchase(request):
    if request.POST:
        ...
        if purchase.is_valid():
            if purchase.save():
                return redirect('/purchase',
                                messages.success(request, 'New Purchase was successfully created.', 'alert-success'))
            else:
...
def editPurchase(request, purchase_id):
    ...
    if request.POST:
        ...
        if purchase.is_valid():
            if purchase.save():
                return redirect('/purchase',
                                messages.success(request, 'Purchase was successfully updated.', 'alert-success'))
            else:
                return redirect('/purchase', messages.error(request, 'Data is not saved', 'alert-danger'))
        else:
            return redirect('/purchase', messages.error(request, 'Purchase is not valid', 'alert-danger'))
...
def new_part(request):
    if request.POST:
        ...
        if part.is_valid():
            if part.save():
                return redirect('/purchase/indexpart',
                                messages.success(request, 'Part was successfully created.', 'alert-success'))
            else:
...
def new_account(request):
    if request.POST:
        ...
        if account.is_valid():
            if account.save():
                return redirect('/purchase/indexaccount',
                                messages.success(request, 'Account was successfully created.', 'alert-success'))
            else:
                return redirect('/purchase/indexaccount',
                                messages.error(request, 'Account is not saved', 'alert-danger'))
        else:
...
def destroy_part(request, part_id):
    if Part.objects.filter(id=part_id).update(active='0'):
        return redirect('/purchase/indexpart',
                        messages.success(request, 'Part was successfully deleted.', 'alert-success'))
    else:
        return redirect('/purchase/indexpart',
                        messages.danger(request, 'Cannot delete Part while its order exists.', 'alert-danger'))
...
def destroy_account(request, account_id):
    if Account.objects.filter(id=account_id).update(active='0'):
        return redirect('/purchase/indexaccount',
                        messages.success(request, 'Account was successfully deleted.', 'alert-success'))
    else:
        return redirect('/purchase/indexaccount',
                        messages.danger(request, 'Cannot delete Account while its order exists.', 'alert-danger'))
...
def new_price(request):
    if request.POST:
        ...
        if price.is_valid():
            if price.save():
                return redirect('/order/indexprice',
                                messages.success(request, 'Price was successfully created.', 'alert-success'))
            else:
...
def new_billet(request):
    if request.POST:
        ...
        if billet.is_valid():
            if billet.save():
                return redirect('/order/indexbillet',
                                messages.success(request, 'Billet data was successfully entered.', 'alert-success'))
            else:
                return redirect('/order/indexbillet',
                                messages.error(request, 'Billet data is not saved', 'alert-danger'))
        else:
...
def editBillet(request, billet_id):
    ...
    if request.POST:
        ...
        if billet.is_valid():
            if billet.save():
                return redirect('/order/indexbillet',
                                messages.success(request, 'New Billet data was successfully updated.', 'alert-success'))
            else:
...
def destroyBillet(request, billet_id):
    ...
    billet.delete()
    return redirect('/order/indexbillet',
                    messages.success(request, 'Billet Data was successfully deleted.', 'alert-success'))
...
def new_log(request):
    if request.POST:
        ...
        if log.is_valid():
            if log.save():
                return redirect('/order/indexlog',
                                messages.success(request, 'New Log data was successfully created.', 'alert-success'))
            else:
...
def editLog(request, log_id):
    ...
    if request.POST:
        ...
        if log.is_valid():
            if log.save():
                return redirect('/order/indexlog',
                                messages.success(request, 'New Log data was successfully updated.', 'alert-success'))
            else:
                return redirect('/order/indexlog', messages.error(request, 'Log data is not saved', 'alert-danger'))
        else:
            return redirect('/order/indexlog', messages.error(request, 'Log data is not valid', 'alert-danger'))
...
def new_revenue(request):
    if request.POST:
        ...
        if revenue.is_valid():
            if revenue.save():
                return redirect('/order/indexrevenue',
                                messages.success(request, 'New Revenue data was successfully created.',
                                                 'alert-success'))
            else:
                return redirect('/order/indexrevenue',
                                messages.error(request, 'Revenue data is not saved', 'alert-danger'))
        else:
...
def editRevenue(request, revenue_id):
    ...
    if request.POST:
        ...
        if revenue.is_valid():
            if revenue.save():
                return redirect('/order/indexrevenue',
                                messages.success(request, 'New Revenue data was successfully updated.',
                                                 'alert-success'))
            else:
                return redirect('/order/indexrevenue',
                                messages.error(request, 'Revenue data is not saved', 'alert-danger'))
...
def destroyRevenue(request, revenue_id):
    ...
    revenue.delete()
    return redirect('/order/indexrevenue',
                    messages.success(request, 'Revenue Data was successfully deleted.', 'alert-success'))
...
def revenue_chart(request):
    ...
    summary_gross_revenue = summary_initial.annotate(month=TruncMonth('date')).values('month').annotate(
        total=Round(Sum('revenue'))).order_by()
...
def new_foundry(request):
    if request.POST:
        ...
        if foundry.is_valid():
            if foundry.save():
                return redirect('/order/indexfoundry',
                                messages.success(request, 'New Foundry data was successfully created.',
                                                 'alert-success'))
            else:
                return redirect('/order/indexfoundry',
                                messages.error(request, 'Foundry data is not saved', 'alert-danger'))
        else:
            print(log.errors)
...
def editFoundry(request, foundry_id):
    ...
    if request.POST:
        ...
        if foundry.is_valid():
            if foundry.save():
                return redirect('/order/indexfoundry',
                                messages.success(request, 'New Foundry data was successfully updated.',
                                                 'alert-success'))
            else:
                return redirect('/order/indexfoundry',
                                messages.error(request, 'Foundry data is not saved', 'alert-danger'))
...
def destroyFoundry(request, foundry_id):
    ...
    foundry.delete()
    return redirect('/order/indexfoundry',
                    messages.success(request, 'Foundry Data was successfully deleted.', 'alert-success'))
...
def edit_receiving(request, receiving_id):
    ...
    if request.POST:
        ...
        receiving.save()
        return redirect('/order/allreceiving',
                        messages.success(request, 'New Receiving data was successfully created.', 'alert-success'))
    else:
        receiving = OrderForm(instance=receiving)
        response = get_value(request.user.id)
        return render(request, 'newreceiving.html',
                      {'receiving': receiving, 'response': response, 'receiving_id': receiving_id})


@login_required
def index_debt(request):
    debt = Order.objects.values('customer_id__customer_number', 'customer_id__customer_name').annotate(
        total=Round(Sum('order_value') - Sum('amount_received'))).order_by()
...
def edit_shipping(request, shipping_id):
    ...
    if request.POST:
        ...
        shipping.save()
        return redirect('/order/allshipping',
                        messages.success(request, 'New Shipping data was successfully created.', 'alert-success'))
    else:
...
def maintenance(request):
    if request.POST:
        ...
        if maintenance.is_valid():
            if maintenance.save():
                return redirect('/order/indexmaintenance',
                                messages.success(request, 'New Maintenance data was successfully created.',
                                                 'alert-success'))
            else:
                return redirect('/order/indexmaintenance',
                                messages.error(request, 'Maintenance data is not saved', 'alert-danger'))
        else:
            return redirect('/order/indexmaintenance',
                            messages.error(request, 'Maintenance data is not valid', 'alert-danger'))
    else:
...
def edit_maintenance(request, maintenance_id):
    ...
    if request.POST:
        ...
        if maintenance.is_valid():
            if maintenance.save():
                return redirect('/order/indexmaintenance',
                                messages.success(request, 'New Maintenance data was successfully updated.',
                                                 'alert-success'))
            else:
                return redirect('/order/indexmaintenance',
                                messages.error(request, 'Maintenance data is not saved', 'alert-danger'))
...
def delete_maintenance(request, maintenance_id):
    ...
    maintenance.delete()
    return redirect('/order/indexmaintenance',
                    messages.success(request, 'Maintenance data was successfully deleted.', 'alert-success'))
...
def edit_payment(request, payment_id):
    ...
    if request.POST:
        ...
        payment.save()
        return redirect('/order/allpayment',
                        messages.success(request, 'New Payment data was successfully created.', 'alert-success'))
    else:
...
def edit_storage(request, storage_id):
    ...
    if request.POST:
        ...
        storage.save()
        return redirect('/order/allstorage',
                        messages.success(request, 'New Payment data was successfully created.', 'alert-success'))
    else:
...
def add_injection(request):
    if request.POST:
        ...
        if injection.is_valid():
            if injection.save():
                return redirect('/order/all_injection',
                                messages.success(request, 'New Injection data was successfully created.',
                                                 'alert-success'))
            else:
                return redirect('/order/all_injection',
                                messages.error(request, 'Injection data is not saved', 'alert-danger'))
        else:
            return redirect('/order/all_injection',
                            messages.error(request, 'Injection data is not valid', 'alert-danger'))
    else:
...
def edit_injection(request, injection_id):
    ...
    if request.POST:
        ...
        if injection.is_valid():
            if injection.save():
                return redirect('/order/all_injection',
                                messages.success(request, 'New Injection data was successfully updated.',
                                                 'alert-success'))
            else:
                return redirect('/order/all_injection',
                                messages.error(request, 'Injection data is not saved', 'alert-danger'))
...
def delete_injection(request, injection_id):
    ...
    injection.delete()
    return redirect('/order/all_injection',
                    messages.success(request, 'Injection data was successfully deleted.', 'alert-success'))
